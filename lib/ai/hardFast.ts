...
export const benchmarkTimings = {
  ...
  simulateForwardMs: 0,
  /** Within the simulation loop: just the chooseGreedyAiAction call itself -- the AI actually deciding a simulated turn's move (a full placement-candidate scoring pass, own estimateMargin scan). */
  simulateChooseMs: 0,
  ...
  simulateApplyMs: 0,
  /** How many individual simulated turns (chooseGreedyAiAction calls) ran inside simulateForward loops, across every sample -- for a per-turn average alongside samples/decisions. */
  simulatedTurns: 0,
...
/**
 * Starting point copied verbatim from twoPly.ts's DEFAULT_TWO_PLY_OPTIONS -- the
 * AI-Arena-validated config (~3-4 point win-rate edge over Medium, n=400/n=1236).
 * Change this (or the algorithm above/below) as speed experiments progress; twoPly.ts
 * itself stays fixed as the baseline to diff against.
 */
...
function evaluateCandidateOnce(state: GameState, playerId: string, candidate: { instanceId: string; position: Position }, roundsAhead: number, rng: Rng): number {
  ...
  t = performance.now();
  let s = resolvePendingVotes(applyAction(determinized, { type: "place", playerId, ...candidate }, rng), rng);
